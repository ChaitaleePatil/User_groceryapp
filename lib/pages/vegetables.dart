...
class _VegetablesPageState extends State<VegetablesPage> {
  ...
  Future<List<QueryDocumentSnapshot>> _fetchItems() async {
    List<QueryDocumentSnapshot> vegetables = [];
    try {
      ...
      for (var doc in shopmenuDocs.docs) {
        ...
        QuerySnapshot itemsSnapshot =
            await itemsCollection.where('category', isEqualTo: 'Vegetables').get();
        vegetables.addAll(itemsSnapshot.docs);
      }
    } catch (e) {
              ...
              builder: (context, snapshot) {
                ...
                return GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return buildVegetableCard(vegetable);
                  },
                );
              },
            ),
  ...
  Widget buildVegetableCard(QueryDocumentSnapshot vegetable) {
    Map<String, dynamic> vegetableData = vegetable.data() as Map<String, dynamic>;
    String productName = vegetableData['productName'] ?? 'No Name';
    String imageUrl = vegetableData['img'] ??
        'https://zaimiaoemrivlpmnujvm.supabase.co/storage/v1/object/public/items/shop_imgs/images.jpeg';
    double price = vegetableData['price']?.toDouble() ?? 0.0;

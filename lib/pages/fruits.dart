...
class _FruitsPageState extends State<FruitsPage> {
  ...
  Future<List<QueryDocumentSnapshot>> _fetchItems() async {
    List<QueryDocumentSnapshot> fruits = [];
    try {
      ...
      for (var doc in shopmenuDocs.docs) {
        ...
        QuerySnapshot itemsSnapshot =
            await itemsCollection.where('category', isEqualTo: 'Fruits').get();
        fruits.addAll(itemsSnapshot.docs);
      }
    } catch (e) {
  ...
  Widget buildFruitCard(QueryDocumentSnapshot fruit) {
    Map<String, dynamic> fruitData = fruit.data() as Map<String, dynamic>;
    String productName = fruitData['productName'] ?? 'No Name';
    String imageUrl = fruitData['img'] ??
        'https://zaimiaoemrivlpmnujvm.supabase.co/storage/v1/object/public/items/shop_imgs/images.jpeg';
    double price = fruitData['price']?.toDouble() ?? 0.0;
